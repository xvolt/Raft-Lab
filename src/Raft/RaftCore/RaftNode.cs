    ...
    public class RaftNode {
        ...
        public Result<bool> AppendEntries(int term, uint leaderId, int prevLogIndex, int prevLogTerm, 
            ...
            if (term < this.CurrentTerm) {
                Logger.Log($"Received AppendEntries with outdated term. Declining.");
                return new Result<bool>(false, CurrentTerm);
            }
            ...
            if (entries != null)  {
                ...
                Logger.Log("Node " + NodeId + " appending new entry " + entries[0].Command);
            }
            else { // HEARTBEAT
                Logger.Log($"Node {NodeId} received heartbeat from {leaderId} (term={term})");
            }


            if (leaderCommit > CommitIndex) {
                //TODO: It gets here on heartbeats
                Logger.Log("Node " + NodeId + " applying entries");
                ...
                if (toApply.Count == 0) {
                    Logger.Log("Node " + NodeId + " failed applying entries");
                    return new Result<bool>(false, CurrentTerm);
                }
        ...
        public Result<bool> RequestVote(int term, uint candidateId, int lastLogIndex, int lastLogTerm) {
            ...
            Logger.Log("Node " + candidateId + " is requesting vote from node " + NodeId);
        ...
        private void StartElection() {
            ...
            // Start election
            Logger.Log("A node has started an election: " + NodeId + " (term " + CurrentTerm + ")");
            ...
            if (voteCount >= GetMajority()) {
                Logger.Log("New leader!! : " + NodeId + " with " + voteCount + " votes");
                LeaderId = NodeId;
        ...
        public void Restart() {
            if (NodeState == NodeState.Stopped) {
                Logger.Log("Restarting node " + NodeId);
                NodeState = NodeState.Follower;
                Run();
            }
        }
        ...
        public void Stop() {
            if (NodeState != NodeState.Stopped) {
                Logger.Log("Bringing node " + NodeId + " down");
        ...
        private void ResetElectionTimer() {
            if (NodeState != NodeState.Leader) {
                electionTimer.Change(TimeSpan.FromMilliseconds( ElectionTimeoutMS), TimeSpan.FromMilliseconds(ElectionTimeoutMS));
            }
        }

        private void ResetHeartbeatTimer() {
            if (NodeState == NodeState.Leader) {
                heartbeatTimer.Change(TimeSpan.FromMilliseconds(ElectionTimeoutMS / 2), TimeSpan.FromMilliseconds(ElectionTimeoutMS /2));
            }
        }
        ...
        private void SendHeartbeats(object arg) {
            ...
            Parallel.ForEach(nodes, nodeId => 
            {
                ...
                if (Log.Count > NextIndex[nodeId]) {
                    ...
                    Logger.Log($"Log Count: {Log.Count} -- Target node[nextIndex]: {nodeId} [{ NextIndex[nodeId]}]. Entries for node {nodeId} count={entries.Count}");
                }
                else {
                ...
                if (res.Value) {
                    if (entries != null) {
                        // Entry appended
                        Logger.Log("Successful AE to " + nodeId + ". Setting nextIndex to " + NextIndex[nodeId]);
                        NextIndex[nodeId] = Log.Count;
                        MatchIndex[nodeId] = Log.Count - 1;
                    }
                }
                else {
                    Logger.Log("Failed AE to " + nodeId + ". Setting nextIndex to " + NextIndex[nodeId]);
